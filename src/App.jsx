...
Based on this data, help me analyze my work performance.
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-700">
      ...
      {/* ✅ TOAST END */}
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 px-3 sm:px-6 py-3 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2">
          <div className="bg-indigo-600 p-1.5 rounded-lg text-white shadow-md shadow-indigo-200"><Clock size={16} /></div>
          <div className="flex flex-col leading-none">
            <h1 className="text-sm sm:text-xl font-black italic tracking-tighter uppercase">
              Time<span className="text-indigo-600">Track</span>
            </h1>
            <button
              onClick={() => setShowAiOptions(true)}
              className="text-[10px] sm:text-[13px] font-semibold text-indigo-500 hover:text-indigo-700 hover:underline transition-all duration-200 cursor-pointer w-fit"
            >
              Ask Ai about your data
            </button>
          </div>
          <button onClick={() => setIsSettingsOpen(true)} className="ml-1 p-1.5 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 rounded-full transition-all duration-300 active:rotate-90 cursor-pointer"><Settings size={16} /></button>
        </div>
        <div className="flex items-center gap-1 bg-slate-100/50 p-0.5 rounded-lg border border-slate-200">
          <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-1 hover:bg-white hover:shadow-sm rounded-md transition-all active:scale-90 cursor-pointer"><ChevronLeft size={14} /></button>
          <span className="px-2 font-bold text-[11px] sm:text-xs min-w-[80px] text-center">{format(currentMonth, 'MMM yyyy')}</span>
          <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-1 hover:bg-white hover:shadow-sm rounded-md transition-all active:scale-90 cursor-pointer"><ChevronRight size={14} /></button>
        </div>
      </nav>
  ...
  ${configSaving
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-4 animate-in slide-in-from-bottom-4 duration-700">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 mb-4">
          ...
          />
          <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-amber-200 hover:shadow-xl hover:shadow-amber-500/5 transition-all duration-500">
            <div className="p-2 sm:p-3 w-fit rounded-xl mb-2 transition-transform group-hover:scale-110 group-hover:rotate-12 bg-amber-50 text-amber-600"><Coffee size={16} /></div>
            <div>
              <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Leave Balance</p>
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1">
                  <span className="text-[10px] font-black text-slate-500">CL</span>
                  <span className="text-lg sm:text-xl font-black tracking-tighter text-amber-600">{Math.floor(config.clHours || 0)}h</span>
                </div>
                <div className="w-[1px] h-4 bg-slate-100"></div>
                <div className="flex items-center gap-1">
                  <span className="text-[10px] font-black text-slate-500">EL</span>
                  <span className="text-lg sm:text-xl font-black tracking-tighter text-amber-600">{Math.floor(config.elHours || 0)}h</span>
                </div>
              </div>
            </div>
          </div>
          <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col justify-between h-full">
            <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
              Earnings
            </p>

            <p className={`text-lg sm:text-2xl font-black tracking-tighter mb-2 ${showEarnings ? "text-slate-800" : "text-slate-400"
              }`}>
              {showEarnings
            ...
            {!showEarnings ? (
              <button
                ...
                className="mt-auto px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-[10px] font-bold cursor-pointer hover:bg-indigo-700 transition-all"
              >
                Show
              </button>
            ) : (
              <button
                ...
                className="mt-auto px-3 py-1.5 bg-gray-300 rounded-lg text-[10px] font-bold cursor-pointer hover:bg-gray-400 transition-all"
              >
                Hide
              </button>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className="lg:col-span-4">
            <div className={`bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border-2 shadow-xl sticky top-16 transition-all duration-500 ${editingId ? 'border-orange-400 ring-4 ring-orange-400/5' : 'border-slate-200 hover:border-indigo-200'}`}>
  ...
  ${saving
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center gap-1.5 bg-white/50 backdrop-blur-sm p-1.5 rounded-xl border border-slate-200 w-fit shadow-sm">
              <button onClick={() => setFilterType('all')} className={`px-3 sm:px-5 py-2 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all duration-300 active:scale-95 flex items-center gap-1.5 cursor-pointer hover:shadow-md ${filterType === 'all' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:bg-white hover:text-slate-600'}`}>
                All Entries <span className={`px-1.5 py-0.5 rounded text-[8px] transition-colors ${filterType === 'all' ? 'bg-slate-700' : 'bg-slate-100'}`}>{filteredLogs.length}</span>
              </button>
              <button onClick={() => setFilterType('short')} className={`px-3 sm:px-5 py-2 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all duration-300 active:scale-95 flex items-center gap-1.5 cursor-pointer hover:shadow-md ${filterType === 'short' ? 'bg-rose-500 text-white shadow-lg shadow-rose-200' : 'text-slate-400 hover:bg-white hover:text-rose-500'}`}>
                Short Leaves <span className={`px-1.5 py-0.5 rounded text-[8px] transition-colors ${filterType === 'short' ? 'bg-rose-700' : 'bg-slate-100'}`}>
            ...
            <div className="bg-white rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden hover:border-indigo-100 transition-colors duration-500">
              <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-wider sm:tracking-widest">
                    <th className="px-3 sm:px-6 py-3 sm:py-4">Work Day</th>
                    <th className="px-3 sm:px-6 py-3 sm:py-4">Time / Short Leave</th>
                    <th className="px-2 sm:px-6 py-3 sm:py-4">Status</th>
                    <th className="px-2 sm:px-6 py-3 sm:py-4 text-right"></th>
                  </tr>
                </thead>
                    ...
                    .map(log => {
                      ...
                      return (
                        <tr key={log.id} className={`hover:bg-indigo-50/30 group transition-all duration-300 ${editingId === log.id ? 'bg-orange-50/50' : ''}`}>
                          <td className="px-3 sm:px-6 py-3 sm:py-4 transition-transform group-hover:translate-x-0.5">
                            <span className="font-bold text-xs sm:text-sm text-slate-800">{format(new Date(log.date), 'do MMM')}</span>
                            <span className="block text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase">{format(new Date(log.date), 'EEE')}</span>
                          </td>
                          <td className="px-3 sm:px-6 py-3 sm:py-4">
                            <div className={`font-mono text-[11px] sm:text-sm font-semibold mb-1 transition-colors ${log.isCL ? 'text-indigo-500 italic' : 'text-slate-600 group-hover:text-slate-900'}`}>{log.inTime && log.outTime
                              ? `${log.inTime} ${log.inPeriod || ''} - ${log.outTime} ${log.outPeriod || ''}`
                              : "Leave"}</div>
                            {showApplyLeave && (
                              <div className="relative animate-in zoom-in duration-300">
                                {shortLeaveSelection.id !== log.id ? (
                                  <button onClick={() => setShortLeaveSelection({ id: log.id, type: '', hours: 0 })} className="flex items-center gap-1 px-2 py-1 rounded-md text-[8px] sm:text-[9px] font-black uppercase tracking-wider transition-all border bg-white border-rose-100 text-rose-500 hover:bg-rose-500 hover:text-white hover:shadow-lg active:scale-95">
                                    <Coffee size={10} /> Apply Leave
                                  </button>
                                ) : (
                                  <div className="flex flex-col gap-1.5 p-2 bg-slate-50 rounded-lg border border-slate-200 mt-1 shadow-inner">
                                    <div className="flex gap-1.5">
                                      {['CL', 'EL'].map(t => {
                                        const balance = t === 'CL' ? config.clHours : config.elHours;
                                        if (balance <= 0) return null;
                                        return <button key={t} onClick={() => setShortLeaveSelection(p => ({ ...p, type: t }))} className={`px-2 py-0.5 rounded text-[9px] font-bold transition-all ${shortLeaveSelection.type === t ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'bg-white text-slate-500 border border-slate-200 hover:border-indigo-300'}`}>{t}</button>;
                                      })}
                                      <button onClick={() => setShortLeaveSelection({ id: null, type: '', hours: 0 })} className="ml-auto text-slate-400 hover:text-rose-500"><X size={12} /></button>
                                    </div>
                                    {shortLeaveSelection.type && (
                                      <select className="text-[9px] p-1.5 rounded border border-slate-200 font-bold outline-none focus:ring-2 focus:ring-indigo-500 animate-in slide-in-from-top-1" onChange={(e) => applyShortLeave(log.id, shortLeaveSelection.type, Number(e.target.value))}>
                            ...
                            {log.shortLeaveType && <div className="text-indigo-600 font-black text-[8px] sm:text-[9px] uppercase tracking-wider flex items-center gap-1 animate-in fade-in"><Check size={10} className="animate-bounce" /> {log.appliedShortLeaveHours || 0}h {log.shortLeaveType} Applied</div>}
                          </td>
                          <td className="px-2 sm:px-6 py-3 sm:py-4">
                            <span className={`inline-block px-2 py-1 rounded-lg text-[8px] sm:text-[10px] font-black uppercase tracking-wider transition-all duration-300 leading-tight ${log.shortLeaveMins ? 'bg-indigo-100 text-indigo-700 shadow-sm' : (diffMins >= 0 ? 'bg-emerald-100 text-emerald-700 shadow-sm' : 'bg-rose-100 text-rose-700')}`}>
                          ...
                          <td className="px-2 sm:px-6 py-3 sm:py-4 text-right">
                            <div className="flex justify-end gap-0.5 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300">
                              <button onClick={() => handleEditInit(log)} className="p-1.5 text-slate-400 hover:bg-white hover:text-indigo-600 hover:shadow-sm rounded-lg transition-all active:scale-90 cursor-pointer"><Pencil size={14} /></button>
                              <button onClick={() => handleDelete(log.id)} className="p-1.5 text-rose-300 hover:bg-white hover:text-rose-600 hover:shadow-sm rounded-lg transition-all active:scale-90 cursor-pointer"><Trash2 size={14} /></button>
                            </div>
                          </td>
                        </tr>
                      );
...
const StatCard = ({ title, value, icon: Icon, color, bg, borderColor, entryBasedDiffMins,
  perDayAdjustment }) => (
  <div className={`bg-white px-3 sm:px-4 py-3 sm:py-4 rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col items-start gap-0.5 group transition-all duration-500 hover:shadow-xl hover:-translate-y-1 ${borderColor || 'hover:border-indigo-200'}`}>
    <div className={`p-2 sm:p-2.5 rounded-xl mb-0.5 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-sm ${bg} ${color}`}><Icon size={16} /></div>
    <div className="w-full">
      <p className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-wider sm:tracking-widest">
        {title}
      </p>
      <p className={`text-base sm:text-xl lg:text-2xl font-black tracking-tighter whitespace-nowrap ${color}`}>
        {value}
      </p>
      {title === "Late Hours" && entryBasedDiffMins !== 0 && (
        <div className="text-[10px] sm:text-[12px] mt-0.5 text-slate-500 font-semibold leading-tight">
          <div>
